...
from models import db, User, Event, Membership, Resume, Match, UserInteraction, EventSession, MeetingLocation, ParticipantAvailability
...
@app.route('/event/<int:event_id>/sessions', methods=['GET', 'POST'])
@login_required
def manage_sessions(event_id):
    # Check if user is admin
    if not current_user.is_administrator():
        flash('Access denied! Only Event Managers can manage sessions.', 'error')
        return redirect(url_for('dashboard'))
...
@app.route('/event/<int:event_id>/locations', methods=['GET', 'POST'])
@login_required
def manage_locations(event_id):
    # Check if user is admin
    if not current_user.is_administrator():
        flash('Access denied! Only Event Managers can manage locations.', 'error')
        return redirect(url_for('dashboard'))
...
@app.route('/event/<int:event_id>/availability', methods=['GET', 'POST'])
@login_required
def manage_availability(event_id):
    # Prevent super admin from managing availability
    if current_user.is_admin:
...
@app.route('/event/<int:event_id>/allocate', methods=['POST'])
@login_required
def allocate_meetings(event_id):
    # Check if user is admin
    if not current_user.is_administrator():
        flash('Access denied! Only Event Managers can allocate meetings.', 'error')
        return redirect(url_for('dashboard'))
    ...
    return redirect(url_for('manage_sessions', event_id=event_id))
